// Blog posts metadata
// Add new posts here in reverse chronological order (newest first)
// Types: case-study, insight, methodology, framework, reflection, announcement
// Language: 'en' (default) or 'zh-Hant' (Traditional Chinese)
// linkedPost: slug of the corresponding post in another language (for hreflang)
const posts = [
  {
    slug: 'thank-you-email-former-boss-tc',
    title: '深夜長文 — 前老細 K 先生',
    date: '2026-08-15',
    excerpt: '尋晚 send 了封 thank you email 給前老闆 K 先生。一個德國人，外表好 cool，但係我見過最 nice 的領導之一——記得 intern 隨口講想試響 17，兩星期後真的買了俾佢。',
    tags: ['career', 'leadership'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'perplexity-dinner-table-tc',
    title: '女朋友 + Perplexity',
    date: '2026-08-15',
    excerpt: '三個月前教女朋友用 Perplexity，三個月後佢用得叻過我。而家食飯會拎電話出黎，不過唔係撳自己野——係萬事問 Perplexity，輸賭 50 蚊，搵佢做公正人。',
    tags: ['ai-adoption', 'ai-tools'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'dont-study-ai-just-use-it-tc',
    title: '唔好學 AI',
    date: '2026-08-15',
    excerpt: '兩個月同過 150 位朋友一對一 AI 私補，我的總結係：唔好學 AI。用是但一隻 LLM 加 Perplexity 已經好夠，有唔明問佢，叫佢 step by step 教你，跟住做。咁已經行先過八成人。',
    tags: ['ai-adoption', 'coaching', 'productivity'],
    type: 'insight',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'gemini-cantonese-transcript-tc',
    title: '免費 + 廣東話OK：如何用Google AI Studio 將你的錄音轉成文字',
    date: '2026-08-15',
    excerpt: '搵好用的 transcript app 好痛苦——假免費、唔支援廣東話、要搞 API。最近發覺 Google Gemini 係世界級：真免費、廣東話友好、低門檻、跨平台，仲可以直接叫 Gemini 做加工。',
    tags: ['gemini', 'ai-tools', 'productivity'],
    type: 'insight',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'stop-selling-ai-fear-tc',
    title: '最討厭「唔學 AI 就被淘汰」',
    date: '2026-08-15',
    excerpt: '我最討厭「唔學 AI 就被淘汰」的論調。純粹為自己利益販賣恐懼，呢啲唔係叫推廣 AI，只係做 AI 版的吳三桂。對我黎講 AI 係好玩、對生活有幫助、解救 ADHD 的良藥。',
    tags: ['ai-adoption', 'adhd'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'living-ai-hkct-talk-tc',
    title: '港專演講 / 生活式 AI',
    date: '2026-08-15',
    excerpt: '作為超級 I 人，要在 400 幾位觀眾面前演講係以前唔敢想像的事。好榮幸能分享「生活式 AI」理念——Voice mode 激發多巴胺、我手 Gen 我心、按張工作。比起生存式 AI，生活式 AI 先係我睇到的未來。',
    tags: ['speaking', 'ai-adoption', 'adhd'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'five-questions-corporate-ai-training-hong-kong-tc',
    title: '香港企業採購 AI 培訓前，必問供應商的五條問題',
    date: '2026-07-29',
    excerpt: '每間供應商網站的字眼幾乎一樣：practical、hands-on、度身訂造。這份清單幫你在表面相似的說法之間做分辨——而寫這份清單的人，自己就是供應商，每條問題你都可以拿回來問我。',
    tags: ['ai-adoption', 'enterprise', 'corporate-training'],
    type: 'framework',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'five-questions-corporate-ai-training-hong-kong',
  },
  {
    slug: 'ai-train-the-trainer-hong-kong-tc',
    title: '香港 AI Train-the-Trainer 課程：你買的是什麼？付款前必問的五條問題',
    date: '2026-07-29',
    excerpt: '你買的從來不是 AI 知識，而是一套交付系統：可修改的教材、run-of-show、hands-on 帶法、現場出事的應變、證書之後的工作機會——付款前要問的五條問題。',
    tags: ['train-the-trainer', 'ai-adoption', 'workshop'],
    type: 'framework',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'ai-train-the-trainer-hong-kong',
  },
  {
    slug: 'five-questions-corporate-ai-training-hong-kong',
    title: 'Five Questions to Ask Before You Buy Corporate AI Training in Hong Kong',
    date: '2026-07-29',
    excerpt: 'Every provider page says the same things: practical, hands-on, tailored. Five questions that tell them apart -- from someone who sells this, so use the questions against me too.',
    tags: ['ai-adoption', 'enterprise', 'corporate-training'],
    type: 'framework',
    featured: false,
    linkedPost: 'five-questions-corporate-ai-training-hong-kong-tc',
  },
  {
    slug: 'ai-train-the-trainer-hong-kong',
    title: 'How to Choose an AI Train-the-Trainer Program in Hong Kong',
    date: '2026-07-29',
    excerpt: 'Hong Kong is short of people who can deliver AI training properly. What a train-the-trainer program should hand you, the five questions to ask before you pay, and where I stand as someone who runs one.',
    tags: ['train-the-trainer', 'ai-adoption', 'workshop'],
    type: 'framework',
    featured: false,
    linkedPost: 'ai-train-the-trainer-hong-kong-tc',
  },
  {
    slug: 'i-wrote-103-rules-for-my-ai-three-broke',
    title: "I've Written Down 103 Rules for My AI. Three of Them Broke Last Week.",
    date: '2026-07-26',
    excerpt: "I published the guide to building a four-tier AI memory system. Then I spent three weeks grading my own. Three rules broke that were already written down -- and none of them failed because the system couldn't find the rule.",
    tags: ['claude-code', 'ai-agents', 'ai-memory', 'productivity'],
    type: 'reflection',
    featured: false,
  },
  {
    slug: 'knowing-ai-and-using-ai-tc',
    title: '懂 AI 和用 AI，是兩件事',
    date: '2026-07-21',
    excerpt: '一次性的工作坊過後，大約只有 4% 的人會把 AI 真正用下去，九成六兩星期內打回原形。問題不在工具，而在習慣——真正的改變，是一件小事重複很多次。',
    tags: ['ai-adoption'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
  },
  {
    slug: 'my-ai-wrote-badly-in-three-languages',
    title: "Why My AI Wrote Badly in Three Languages (It Wasn't the Model)",
    date: '2026-07-17',
    excerpt: "I asked my AI assistant why its writing is bad in English, Chinese and French. The answer wasn't the model -- it was the templates I fed it months ago, when the models were weaker. We deleted eight of them this week.",
    tags: ['ai-writing', 'claude-code', 'prompts', 'productivity'],
    type: 'reflection',
    featured: false,
    linkedPost: 'my-ai-wrote-badly-in-three-languages-tc',
  },
  {
    slug: 'my-ai-wrote-badly-in-three-languages-tc',
    title: '我問 AI 為甚麼文筆這麼差——答案不是模型',
    date: '2026-07-17',
    excerpt: '問題不在模型，在幾個月前寫下的舊指令：模型每一次升級都更強，舊模板卻一直把文字拉回 2025 年的水平。這星期刪掉了八個問題來源，這篇文章就是第一個測試。',
    tags: ['ai-writing', 'claude-code', 'prompts', 'productivity'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'my-ai-wrote-badly-in-three-languages',
  },
  {
    slug: 'three-us-businesses-one-hong-kong-trainer',
    title: 'Three US Businesses, One Hong Kong Trainer',
    date: '2026-07-10',
    excerpt: 'Between April and June 2026, I ran three remote AI coaching sessions for US businesses -- a realty firm, a skincare founder, and an equipment dealer. Here\'s what makes a single-session coaching call produce something worth the time.',
    tags: ['coaching', 'international', 'hong-kong', 'workflow'],
    type: 'methodology',
    featured: false,
    linkedPost: 'three-us-businesses-one-hong-kong-trainer-tc',
  },
  {
    slug: 'three-us-businesses-one-hong-kong-trainer-tc',
    title: '三間美國公司，一個香港 AI 教練：遙距一對一怎樣才有用',
    date: '2026-07-10',
    excerpt: '2026 年 4 至 6 月，三節來自美國的單次 AI coaching call——沒有課程、沒有後續合約，有用的原因是同一個三步結構：call 前拿到真實任務、工具配對任務、一起做出一個 output。',
    tags: ['coaching', 'international', 'hong-kong', 'workflow'],
    type: 'methodology',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'three-us-businesses-one-hong-kong-trainer',
  },
  {
    slug: 'why-my-best-ai-workshops-have-thirteen-people',
    title: 'Why My Best AI Workshops Have Thirteen People, Not Five Hundred',
    date: '2026-06-21',
    excerpt: 'The industry counts reach. But the AI training sessions where workflow actually changes have thirteen people in them, every screen visible, every problem solved in real time.',
    tags: ['workshop', 'coaching', 'corporate-training', 'ai-adoption', 'workflow'],
    type: 'insight',
    featured: false,
    linkedPost: 'why-my-best-ai-workshops-have-thirteen-people-tc',
  },
  {
    slug: 'why-my-best-ai-workshops-have-thirteen-people-tc',
    title: '為什麼我最好的 AI 工作坊是十三個人，不是五百人',
    date: '2026-06-21',
    excerpt: '十三人的房間為什麼比五百人的 webinar 更能改變 workflow：三個具體機制——每個螢幕都看得見、session 可以為一個人停、參加者用自己的材料——加上一個講清楚的代價。',
    tags: ['workshop', 'coaching', 'corporate-training', 'ai-adoption', 'workflow'],
    type: 'insight',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'why-my-best-ai-workshops-have-thirteen-people',
  },
  {
    slug: 'one-jewellery-brand-two-ai-trainings-in-two-days',
    title: 'One Jewellery Brand, Two AI Trainings in Two Days: Design, Then Operations',
    date: '2026-06-14',
    excerpt: 'A Hong Kong jewellery retailer booked one AI session for its design team, then booked a second for operations the next day. What the two curricula looked like, and what cross-department demand actually signals.',
    tags: ['corporate-training', 'coaching', 'ai-adoption', 'jewellery', 'hong-kong'],
    type: 'case-study',
    featured: false,
  },
  {
    slug: 'what-a-500-person-ai-webinar-can-and-cant-do',
    title: 'What a 500-Person AI Webinar Can and Can\'t Do',
    date: '2026-06-30',
    excerpt: 'I delivered a firmwide AI awareness webinar for a global advertising group in Hong Kong. About 500 people. Here\'s what a broadcast that size can realistically do, what it can\'t, and the three design choices that make it worth running.',
    tags: ['corporate-training', 'advertising', 'speaking', 'ai-adoption'],
    type: 'insight',
    featured: false,
  },
  {
    slug: '180-people-six-sessions-one-pattern',
    title: '180 People, Six Sessions, One Pattern',
    date: '2026-07-04',
    excerpt: 'Across six different AI sessions and 180 participants at one organisation, the pattern was consistent -- the executives were more anxious than their staff. What this means for who you train first.',
    tags: ['corporate-training', 'ai-adoption', 'leadership', 'hong-kong'],
    type: 'insight',
    featured: false,
  },
  {
    slug: 'what-120-ceos-asked-after-the-ai-demo',
    title: 'What 120 CEOs Asked After the AI Demo',
    date: '2026-06-16',
    excerpt: 'At the YPO AI Advantage Summit in Los Angeles, 120 CEOs asked questions that had nothing to do with AI. They asked about creativity, their children, and whether they\'d started too late. Here are the answers.',
    tags: ['corporate-training', 'ypo', 'hong-kong', 'international'],
    type: 'reflection',
    image: '/images/blog/ypo-la-skirball-demo.jpg',
    featured: false,
    linkedPost: 'what-120-ceos-asked-after-the-ai-demo-tc',
  },
  {
    slug: 'what-120-ceos-asked-after-the-ai-demo-tc',
    title: '為 120 位 CEO 示範 AI 之後，他們問的問題與 AI 無關',
    date: '2026-06-16',
    excerpt: '2025 年 11 月，洛杉磯 YPO 峰會上，120 位 CEO 在 AI 示範後提出的問題涉及創意、子女出路與起步時機，而非技術本身。本文整理這些問題的答案。',
    tags: ['corporate-training', 'ypo', 'hong-kong', 'international'],
    type: 'reflection',
    image: '/images/blog/ypo-la-skirball-demo.jpg',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'what-120-ceos-asked-after-the-ai-demo',
  },
  {
    slug: 'food-company-year-of-ai-six-departments',
    title: 'A Year of AI Training at a Hong Kong Food Company: Six Departments, Twenty Sessions',
    date: '2026-06-24',
    excerpt: 'What happens when a company doesn\'t just book one AI workshop, but keeps going -- department by department -- for a full year.',
    tags: ['enterprise', 'food-manufacturing', 'corporate-training', 'workflow'],
    type: 'case-study',
    featured: false,
    linkedPost: 'food-company-year-of-ai-six-departments-tc',
  },
  {
    slug: 'food-company-year-of-ai-six-departments-tc',
    title: '一間香港食品公司的一年 AI 培訓：六個部門，二十節課',
    date: '2026-06-24',
    excerpt: '從一場標準 AI 工作坊開始，該公司六個部門在一年內上了約二十節課，超過一百名員工受訓，5 月評估達 4.48 分。',
    tags: ['enterprise', 'food-manufacturing', 'corporate-training', 'workflow'],
    type: 'case-study',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'food-company-year-of-ai-six-departments',
  },
  {
    slug: 'five-things-every-ai-trainer-should-prepare',
    title: 'Five Things Every AI Trainer Should Prepare -- and the One That Saves You',
    date: '2026-07-02',
    excerpt: 'After enough broken demos and quiet rooms, the preparation for any AI training engagement crystallized into five outputs. Here\'s what they are.',
    tags: ['train-the-trainer', 'corporate-training'],
    type: 'methodology',
    image: '/images/blog/creativity-workshop-la.jpg',
    featured: false,
  },
  {
    slug: 'two-workshops-one-day-parents-kids',
    title: 'Two Workshops, One Day: Parents in the Morning, Kids in the Afternoon',
    date: '2026-06-19',
    excerpt: 'A parent workshop about AI fears at a Hong Kong school in the morning. A children\'s AI build in the afternoon. Same tools, same city, and the working answers I\'ve started giving parents since.',
    tags: ['ai-education', 'families', 'hong-kong', 'ai-adoption'],
    type: 'reflection',
    featured: false,
    linkedPost: 'two-workshops-one-day-parents-kids-tc',
  },
  {
    slug: 'two-workshops-one-day-parents-kids-tc',
    title: '同一日兩場 AI 工作坊：上午教家長，下午教小朋友',
    date: '2026-06-19',
    excerpt: '同一日，上午我在香港一間中學向約一百位家長講 AI，下午二十個孩子用 AI 生成海報；本文是我實際給家長的答案。',
    tags: ['ai-education', 'families', 'hong-kong', 'ai-adoption'],
    type: 'reflection',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'two-workshops-one-day-parents-kids',
  },
  {
    slug: 'what-smes-actually-ask-about-ai',
    title: 'What SMEs Actually Ask About AI',
    date: '2026-06-27',
    excerpt: 'I sat on a panel at an HKPC expo for small businesses. The prepared answers were fine. The audience\'s questions were better — here are the actual answers.',
    tags: ['sme', 'ai-adoption', 'hong-kong', 'hkpc', 'speaking'],
    type: 'insight',
    featured: false,
  },
  {
    slug: 'the-architect-that-never-writes-code',
    title: 'The architect that never writes code',
    date: '2026-07-11',
    excerpt: 'Companion take to the build log: the most expensive model in the stack wrote zero implementation code. Four bugs died at the spec stage, three workers passed the same five gates, and the blind pick landed on the cheapest. The pattern, not the model, is the asset.',
    tags: ['ai-agents', 'ai-tools', 'workflow'],
    type: 'insight',
    featured: false,
    linkedPost: 'the-architect-that-never-writes-code-tc',
  },
  {
    slug: 'the-architect-that-never-writes-code-tc',
    title: '從不寫 code 的 architect model',
    date: '2026-07-11',
    excerpt: '一個從不寫 code 的 architect 模型，統籌 3 個來自不同公司的模型在同一條 git branch 上工作，在 spec 階段就消滅了 4 個 bug，最終產出一份淨刪減的 diff。模型是可以互換的應徵者——orchestration 才是資產。',
    tags: ['ai-agents', 'ai-tools', 'workflow'],
    type: 'insight',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'the-architect-that-never-writes-code',
  },
  {
    slug: 'three-models-one-branch-zero-deployment',
    title: 'Three models, one branch, zero deployment',
    date: '2026-07-10',
    excerpt: 'First live run of the architect pattern on my own website: Claude Fable 5 wrote the specs and verdicts, Grok 4.5 and GPT-5.5 did the typing in parallel. 75 files, +167/−668 — and nothing ships until I decide.',
    tags: ['ai-agents', 'ai-tools', 'ai-adoption', 'workflow'],
    type: 'case-study',
    featured: false,
  },
  {
    slug: 'ai-manager-not-maker',
    title: 'AI made me a manager, not a better maker',
    date: '2026-07-02',
    excerpt: 'AI raised my output and quietly changed the job — from making the thing to deciding what is worth making. Five practitioners circling the same shift, and why taste is the part that did not get automated.',
    tags: ['ai-agents', 'ai-adoption', 'ai-writing'],
    type: 'reflection',
    featured: false,
  },
  {
    slug: 'three-gemini-tools-collapsed-into-one-prompt-today',
    title: 'Three Gemini Tools Collapsed Into One Prompt Today',
    date: '2026-05-20',
    excerpt: 'Google shipped Gemini Omni at I/O 2026 — Veo, Nano Banana, and Genie folded into one multimodal prompt surface. Three of my daily Gemini calls collapse into one. Day one read, API still weeks out, the consolidation thesis lands.',
    tags: ['gemini', 'multimodal-ai', 'ai-tools', 'corporate-training'],
    type: 'insight',
    featured: false,
    linkedPost: 'three-gemini-tools-collapsed-into-one-prompt-today-tc',
  },
  {
    slug: 'three-gemini-tools-collapsed-into-one-prompt-today-tc',
    title: '三個 Gemini 工具，今日合併成一個 prompt',
    date: '2026-05-20',
    excerpt: 'Google 今早在 I/O 2026 發佈 Gemini Omni，把 Veo、Nano Banana 與 Genie 摺進同一個 prompt surface——Sam 日常的三條 Gemini call 收歸一個。未實測，但方向值得 day one 寫下：跨輪持久性仍是關鍵死穴。',
    tags: ['gemini', 'multimodal-ai', 'ai-tools', 'corporate-training'],
    type: 'insight',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'three-gemini-tools-collapsed-into-one-prompt-today',
  },
  {
    slug: 'plus-minus-multiply-divide-copilot-framework',
    title: 'Plus, Minus, Multiply, Divide: A Cheat Sheet for Teaching Copilot',
    date: '2026-05-11',
    excerpt: 'I rewrote a finance Copilot workshop at 1am and reorganised it around four arithmetic moves. The framework outlasted the panic.',
    tags: ['copilot', 'corporate-training'],
    type: 'methodology',
    featured: false,
    linkedPost: 'plus-minus-multiply-divide-copilot-framework-tc',
  },
  {
    slug: 'plus-minus-multiply-divide-copilot-framework-tc',
    title: '加、減、乘、除：教 Copilot 的速查表',
    date: '2026-05-11',
    excerpt: '凌晨 1 點推倒財務工作坊教材，圍繞加減乘除四則運算重新設計。與其教功能導覽，不如給一套記得住的方法論：加 context，減雜訊，用 agent 重複好動作，用 custom instructions 省掉時間稅。3 星期後的星期二，才是真正的測試。',
    tags: ['copilot', 'corporate-training'],
    type: 'methodology',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'plus-minus-multiply-divide-copilot-framework',
  },
  {
    slug: 'prep-job-interview-ai-30-minutes',
    title: 'How to Prep for a Job Interview Using AI in 30 Minutes',
    date: '2026-05-04',
    excerpt: 'A 30-minute AI loop for prepping a job interview — NotebookLM podcasts of your CV, then the company, then a recorded ChatGPT voice-mode mock interview.',
    tags: ['ai-adoption', 'productivity', 'career', 'notebooklm'],
    type: 'methodology',
    featured: false,
    linkedPost: 'prep-job-interview-ai-30-minutes-tc',
  },
  {
    slug: 'prep-job-interview-ai-30-minutes-tc',
    title: '用 AI 30 分鐘準備求職面試',
    date: '2026-05-04',
    excerpt: '用 NotebookLM 生成 podcast、ChatGPT 語音模擬面試，再把錄音掉回 notebook——3 個 source 疊加，循環 2 至 3 輪，30 分鐘一圈，讓大腦在面試的語言裡浸到熟悉。不是背答案，是高速建立接觸效應。',
    tags: ['ai-adoption', 'productivity', 'career', 'notebooklm'],
    type: 'methodology',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'prep-job-interview-ai-30-minutes',
  },
  {
    slug: 'food-company-ai-workshops-three-batches',
    title: 'I\'ve Now Run AI Workshops for the Same Company Three Times. The Third Batch Is Nothing Like the First.',
    date: '2026-04-13',
    excerpt: 'I\'ve been training at a Hong Kong food manufacturer since January: HR, then Sales & Marketing, now Operations. The third batch required a 120-row fake production dataset and 26 new prompts. Nothing from Batch 1 transferred.',
    tags: ['corporate-training', 'ai-adoption', 'workshop'],
    type: 'case-study',
    featured: false,
    linkedPost: 'food-company-ai-workshops-three-batches-tc',
  },
  {
    slug: 'food-company-ai-workshops-three-batches-tc',
    title: '三輪 AI 工作坊，同一間公司——第三輪我一張舊 slide 都沒用',
    date: '2026-04-13',
    excerpt: '1 月起在一間香港食品製造商培訓：HR、市場及銷售，現在是營運。第三輪他放棄全部舊 slide，從零建 120 行虛構生產數據集和 26 條專用 prompt，並坦言未知成效。一間機構的 AI 成熟度不是一個分數，是一幅拼布。',
    tags: ['corporate-training', 'ai-adoption', 'workshop'],
    type: 'case-study',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'food-company-ai-workshops-three-batches',
  },
  {
    slug: 'hong-kong-ai-disadvantage-is-advantage',
    title: 'Why Hong Kong\'s AI Disadvantage Is Actually an Advantage',
    date: '2026-04-13',
    excerpt: 'I trained 120 CEOs in LA who couldn\'t copy and paste. Hong Kong\'s tech literacy is higher than people think. The real gap isn\'t VPNs — it\'s information lag and the Cantonese blind spot.',
    tags: ['ai-adoption', 'hong-kong', 'ai-writing'],
    type: 'insight',
    image: '/images/blog/hkct-ai-workshop.webp',
    featured: true,
  },
  {
    slug: 'when-a-school-commits-to-24-months-of-ai',
    title: 'What Happens When a School Commits to 24 Months of AI Training',
    date: '2026-04-13',
    excerpt: 'Last April I trained 400 educators in 75 minutes. Then HKCT came back for a 24-month AI Learning Community — and mandated NotebookLM for all students and staff by September.',
    tags: ['workshop', 'ai-education', 'ai-adoption'],
    type: 'case-study',
    image: '/images/blog/hkct-ai-workshop.webp',
    featured: true,
  },
  {
    slug: 'i-audited-my-ai-blog-it-was-slop',
    title: 'I Audited My AI-Managed Blog. Half the Posts Were Slop.',
    date: '2026-04-13',
    excerpt: 'I teach people to evaluate AI output critically. Then I read my own blog and found five posts with the same three-point structure, the same filler transitions, and the same closing. Here is what I changed.',
    tags: ['claude-code', 'automation', 'ai-writing', 'productivity'],
    type: 'reflection',
    image: '/images/blog/my-blog-publishes-itself-silent-death-cover.webp',
    featured: true,
  },
  {
    slug: 'my-blog-publishes-itself-silent-death',
    title: 'My Blog Publishes Itself. It Died Silent for 3 Weeks.',
    date: '2026-04-12',
    excerpt: 'I built an autonomous AI blog pipeline. It ran for months, published 47 posts, then silently stopped for three weeks. Exit code 0. No alerts. Here is what went wrong and the three principles I now follow.',
    tags: ['claude-code', 'automation', 'productivity'],
    type: 'reflection',
    image: '/images/blog/my-blog-publishes-itself-silent-death-cover.webp',
    featured: false,
    linkedPost: 'my-blog-publishes-itself-silent-death-tc',
  },
  {
    slug: 'my-blog-publishes-itself-silent-death-tc',
    title: '我的部落格自動發佈文章。然後悄無聲息地停擺了三星期。',
    date: '2026-04-12',
    excerpt: '我建立了一條自動化 AI 內容管線。運行數月，發佈了 47 篇文章，然後悄然停擺三週。退出碼 0。無警報。以下是出了什麼問題，以及我現在遵循的三個原則。',
    tags: ['claude-code', 'automation', 'productivity'],
    type: 'reflection',
    image: '/images/blog/my-blog-publishes-itself-silent-death-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'my-blog-publishes-itself-silent-death',
  },
  {
    slug: '9-percent-copilot-licenses',
    title: "9% of Copilot Licenses Are Actually Used. I'm Not Surprised.",
    date: '2026-03-27',
    excerpt: "Organizations spend millions on AI licenses and zero on training. The adoption gap isn't a tool problem -- it's a training problem wearing a tool costume.",
    tags: ['ai-adoption', 'enterprise', 'corporate-training'],
    type: 'commentary',
    image: '/images/blog/9-percent-copilot-licenses-cover.webp',
    featured: false,
    linkedPost: '9-percent-copilot-licenses-tc',
  },
  {
    slug: '9-percent-copilot-licenses-tc',
    title: '9% 的 Copilot 授權有人使用。我一點也不意外',
    date: '2026-03-27',
    excerpt: 'Copilot 授權只有 9% 曾被啟用，與一次性培訓後僅 4% 持續使用的數字指向同一問題：企業把 99% 預算用來買工具，1% 用來教人。真正留得住的採用，靠的是培訓完結之後仍然有人在。',
    tags: ['ai-adoption', 'enterprise', 'corporate-training'],
    type: 'commentary',
    image: '/images/blog/9-percent-copilot-licenses-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: '9-percent-copilot-licenses',
  },
  {
    slug: 'ai-chatbot-2-weeks-what-broke',
    title: "I Built an AI Chatbot in 2 Weeks. Here's What Broke on Turn 3.",
    date: '2026-03-27',
    excerpt: "The gap between 'build an AI chatbot' tutorials and a working production chatbot is measured in crashes, security holes, and spec rewrites.",
    tags: ['ai-adoption', 'enterprise'],
    type: 'case-study',
    image: '/images/blog/ai-chatbot-2-weeks-what-broke-cover.webp',
    featured: false,
    linkedPost: 'ai-chatbot-2-weeks-what-broke-tc',
  },
  {
    slug: 'ai-chatbot-2-weeks-what-broke-tc',
    title: '兩星期建了一個 AI Chatbot。第三輪對話就崩潰',
    date: '2026-03-27',
    excerpt: '一個人加一隊 AI coding agent，兩星期建好涵蓋 137 間學校的 multi-agent chatbot，卻在第三輪對話死掉。原因是一條檢查組裝 context 而非用戶訊息的 guardrail——修復只需 6 行 code，找到它用了 1 小時。',
    tags: ['ai-adoption', 'enterprise'],
    type: 'case-study',
    image: '/images/blog/ai-chatbot-2-weeks-what-broke-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'ai-chatbot-2-weeks-what-broke',
  },
  {
    slug: 'claude-code-mastery-part-5-full-blueprint',
    title: 'Claude Code Mastery: The Full Blueprint — My Actual System',
    date: '2026-03-23',
    excerpt: 'I open-sourced my entire Claude Code architecture. Every file, every connection, every lesson learned. 9 agents, 40 skills, 7 cron jobs, 4-tier memory — built because I have ADHD and needed external structure.',
    tags: ['claude-code', 'system-design', 'productivity', 'adhd'],
    type: 'case-study',
    image: '/images/blog/claude-code-mastery-part-5-cover.webp',
    featured: false,
    linkedPost: 'claude-code-mastery-part-5-full-blueprint-tc',
  },
  {
    slug: 'claude-code-mastery-part-5-full-blueprint-tc',
    title: 'Claude Code 完全攻略：完整藍圖 — 我真正的系統',
    date: '2026-03-23',
    excerpt: '我將整個 Claude Code 系統完整公開。CLAUDE.md、skills、agents、hooks、cron、memory — 每一層如何連接，每一個決定背後的原因。',
    tags: ['claude-code', 'system-design', 'productivity', 'adhd'],
    type: 'case-study',
    image: '/images/blog/claude-code-mastery-part-5-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'claude-code-mastery-part-5-full-blueprint',
  },
  {
    slug: 'claude-code-mastery-part-4-agent-teams',
    title: 'Claude Code Mastery: Agent Teams & Automation — Your AI Workforce',
    date: '2026-03-21',
    excerpt: 'I have 9 AI agents running 7 cron jobs. My blog publishes itself every Monday. Here is the full agent team architecture — delegation rules, parallel execution, and cost-aware model selection.',
    tags: ['claude-code', 'ai-agents', 'automation'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-4-cover.webp',
    featured: false,
    linkedPost: 'claude-code-mastery-part-4-agent-teams-tc',
  },
  {
    slug: 'claude-code-mastery-part-4-agent-teams-tc',
    title: 'Claude Code 完全攻略：Agent 團隊與自動化 — 你的 AI 工作團隊',
    date: '2026-03-21',
    excerpt: '9 個 agent、7 個 cron job：blog 每星期一自動發文，每個平日 8:30 收到廣東話簡報。這篇拆解單一 agent 為何會崩、平行與順序執行怎樣選，以及讓 9 個 agent 不燒錢的模型路由。',
    tags: ['claude-code', 'ai-agents', 'automation'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-4-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'claude-code-mastery-part-4-agent-teams',
  },
  {
    slug: 'claude-code-mastery-part-3-hooks-guardrails',
    title: 'Claude Code Mastery: Hooks & Guardrails — Making AI Safe',
    date: '2026-03-20',
    excerpt: 'My AI agent nearly sent an email to the wrong person. That one near-miss changed everything. Here is the hooks system I built — PreToolUse validation, prompt injection scanning, and a policy engine.',
    tags: ['claude-code', 'ai-safety', 'system-design', 'automation'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-3-cover.webp',
    featured: false,
    linkedPost: 'claude-code-mastery-part-3-hooks-guardrails-tc',
  },
  {
    slug: 'claude-code-mastery-part-3-hooks-guardrails-tc',
    title: 'Claude Code 完全攻略：Hooks 與安全機制 — 如何讓 AI 安全工作',
    date: '2026-03-20',
    excerpt: '一封差點發給錯誤客戶的 invoice email，被 PreToolUse hook 攔下：損失 12 秒，傷害是零。這篇拆解整套護欄——163 行 policy engine、批准規則、記憶強制回寫、prompt injection 防禦。',
    tags: ['claude-code', 'ai-safety', 'system-design', 'automation'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-3-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'claude-code-mastery-part-3-hooks-guardrails',
  },
  {
    slug: 'claude-code-mastery-part-2-skills-memory',
    title: 'Claude Code Mastery: Skills & Memory — Teaching AI to Remember',
    date: '2026-03-19',
    excerpt: 'AI forgets everything between sessions. I solved this with a 4-tier memory system and 40 active skills that load in 100 tokens each. Here is exactly how persistent knowledge works in Claude Code.',
    tags: ['claude-code', 'ai-memory', 'productivity'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-2-cover.webp',
    featured: false,
    linkedPost: 'claude-code-mastery-part-2-skills-memory-tc',
  },
  {
    slug: 'claude-code-mastery-part-2-skills-memory-tc',
    title: 'Claude Code 完全攻略：Skills 與 Memory — 讓 AI 學會記憶',
    date: '2026-03-19',
    excerpt: '花 45 分鐘教會 AI，第二天它全忘掉——Sam 稱之為金魚問題。他建了四層記憶系統加 40 個按需載入的 skill，開機載入不超過 5,000 token，讓 AI 運作得像一個共事了幾個月的同事。',
    tags: ['claude-code', 'ai-memory', 'productivity'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-2-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'claude-code-mastery-part-2-skills-memory',
  },
  {
    slug: 'claude-code-mastery-part-1-getting-started',
    title: 'Claude Code Mastery: From Zero to Your First Agent',
    date: '2026-03-17',
    excerpt: 'Three months ago I opened Claude Code and thought it was just another terminal. Today it runs my entire business. Part 1 of a series on how I actually use it — installation, mental model, CLAUDE.md, and your first real task.',
    tags: ['claude-code', 'ai-tools', 'productivity'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-1-cover.webp',
    featured: true,
    linkedPost: 'claude-code-mastery-part-1-getting-started-tc',
  },
  {
    slug: 'claude-code-mastery-part-1-getting-started-tc',
    title: 'Claude Code 完全攻略：從零開始到你的第一個 Agent',
    date: '2026-03-17',
    excerpt: '3 個月前 Sam 以為 Claude Code 只是另一個 terminal chatbox；3 個月後它承載 4 間公司、70+ 客戶項目。系列第 1 篇由零開始：安裝、六個工具的 mental model、plan mode，以及一份告訴 agent 它是誰的 CLAUDE.md。',
    tags: ['claude-code', 'ai-tools', 'productivity'],
    type: 'methodology',
    image: '/images/blog/claude-code-mastery-part-1-cover.webp',
    language: 'zh-Hant',
    featured: false,
    linkedPost: 'claude-code-mastery-part-1-getting-started',
  },
  {
    slug: 'ai-change-management-what-actually-works',
    title: 'AI Change Management: What Actually Works (A Field Report From 70+ Organizations)',
    date: '2026-03-23',
    excerpt: 'Most companies approach AI adoption like a software rollout. After training 10,000+ professionals across 70+ organizations, here is what actually drives lasting behavior change.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'methodology',
    image: '/images/blog/ai-change-management-cover.webp',
    featured: false,
  },
  {
    slug: 'agent-adoption-ladder',
    title: 'The Agent Adoption Ladder: What Most People Get Wrong About AI Agents',
    date: '2026-03-17',
    excerpt: 'I taught my non-technical co-founder about AI agents. What came out is a four-tier framework for matching the right agent tool to how you actually work — from chatbots to multi-agent orchestration.',
    tags: ['ai-adoption', 'ai-agents', 'enterprise'],
    type: 'framework',
    image: '/images/blog/agent-adoption-ladder-cover.webp',
    featured: true,
  },
  {
    slug: 'what-nobody-tells-you-building-agentic-ai',
    title: 'What Nobody Tells You About Building Agentic AI',
    date: '2026-03-16',
    excerpt: 'I built a multi-agent AI system from scratch -- four specialists, a team router, persistent memory, and streaming UX. Here are five things the framework docs don\'t cover.',
    tags: ['ai-adoption', 'ai-agents', 'ai-tools'],
    type: 'insight',
    image: '/images/blog/agentic-ai-lessons-cover.webp',
    featured: false,
    linkedPost: 'zh-agentic-ai-five-lessons',
  },
  {
    slug: 'zh-agentic-ai-five-lessons',
    title: '從零建構一個 Agentic AI 系統學到的五件事',
    date: '2026-03-16',
    excerpt: '從零建構 multi-agent 系統的 5 個教訓：routing 指令要寫足 3 次才穩定、記憶必須共享、把內部運作 surface 到介面反而建立信任、guardrail 內藏文化偏見、framework 只給你 80%——剩下的 20% 才是產品。',
    tags: ['ai-adoption', 'ai-agents', 'ai-tools'],
    type: 'insight',
    language: 'zh-Hant',
    image: '/images/blog/agentic-ai-lessons-cover.webp',
    featured: false,
    linkedPost: 'what-nobody-tells-you-building-agentic-ai',
  },
  {
    slug: 'seo-rebuild-one-day-claude-code',
    title: 'I Rebuilt My Entire SEO in One Day. Here\'s What Actually Moved the Needle.',
    date: '2026-03-15',
    excerpt: 'How I used Claude Code to rebuild my entire SEO and GEO strategy in one session -- structured data, internal linking, hreflang, and pillar pages across 30+ blog posts in 7 hours.',
    tags: ['ai-adoption', 'productivity', 'ai-tools'],
    type: 'insight',
    image: '/images/blog/seo-rebuild-cover.webp',
    featured: false,
  },
  {
    slug: 'how-to-choose-ai-training-hong-kong',
    title: 'Corporate AI Training in Hong Kong: The Five Options Compared (2026)',
    date: '2026-03-15',
    // The funding section was materially wrong (RTTP's ratio, the BUD ceiling)
    // and TVP has been closed since 31 Dec 2024. Corrected 2026-08-12 against
    // itf.gov.hk and tid.gov.hk. datePublished stays honest; dateModified is
    // what tells Google the page is current.
    dateModified: '2026-08-12',
    excerpt: 'Universities, global firms, consultancies, government-subsidised courses, or practitioner-led training -- costs, trade-offs, and what Hong Kong government funding actually covers in 2026, now that TVP is closed and RTTP has become NITTP.',
    tags: ['ai-adoption', 'enterprise', 'corporate-training'],
    type: 'insight',
    image: '/images/blog/how-to-choose-ai-training-hk-cover.webp',
    featured: false,
  },
  {
    slug: 'microsoft-copilot-training-hong-kong',
    title: 'Microsoft Copilot Training for Hong Kong Companies: What Actually Works',
    date: '2026-03-15',
    excerpt: 'Your company already pays for Copilot. Here\'s why adoption is low, what Copilot training should actually cover, and the program structure that produces lasting behavior change.',
    tags: ['ai-adoption', 'enterprise', 'ai-tools'],
    type: 'methodology',
    image: '/images/blog/microsoft-copilot-training-hk-cover.webp',
    featured: false,
  },
  {
    slug: 'why-ai-training-doesnt-stick',
    title: 'Why AI Training Doesn\'t Stick (And What Actually Works)',
    date: '2026-03-15',
    excerpt: 'Most corporate AI training produces zero lasting behavior change. After training 10,000+ professionals across 70+ organizations, here\'s what actually makes AI adoption stick.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'insight',
    image: '/images/blog/why-ai-training-doesnt-stick-cover.webp',
    featured: true,
  },
  {
    slug: 'hk-2026-budget-ai-training',
    title: 'What the 2026 Budget\'s HK$50 Million AI Training Push Actually Means for Your Company',
    date: '2026-03-14',
    excerpt: 'Hong Kong earmarked HK$50 million for AI training in the 2026-27 Budget. Here\'s what it actually means for corporate training, what\'s missing, and what to do now.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'reflection',
    image: '/images/blog/hk-budget-ai-training-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-prompts-beat-procrastination',
    title: '5 AI Prompts That Actually Help With Procrastination (From Someone Who Uses Them Daily)',
    date: '2026-03-14',
    excerpt: 'Five viral AI prompts for procrastination -- why each one works, how to get more from them, and what changes when you have ADHD and use them every day.',
    tags: ['ai-adoption', 'productivity', 'ai-tools'],
    type: 'reflection',
    image: '/images/blog/ai-prompts-procrastination-cover.webp',
    featured: false,
    linkedPost: 'zh-ai-prompts-beat-procrastination',
  },
  {
    slug: 'zh-ai-prompts-beat-procrastination',
    title: '5 條真正有用的 AI Prompt 對付拖延症（我每日在用）',
    date: '2026-03-14',
    excerpt: '5 條在 X 上瘋傳、來自 Reddit 用戶 reclaim_ai 的拖延症 prompt。Sam 有 ADHD，每天在用它們的變化版：逐條拆解為甚麼有效、怎樣用得更好，以及讓 prompt 真正融入日常 workflow 的 3 個方法。',
    tags: ['ai-adoption', 'productivity', 'ai-tools'],
    type: 'reflection',
    language: 'zh-Hant',
    image: '/images/blog/ai-prompts-procrastination-cover.webp',
    featured: false,
    linkedPost: 'ai-prompts-beat-procrastination',
  },
  {
    slug: 'openclaw-ai-agents-talk',
    title: 'What 1,400 People Taught Me About the Gap Between AI Demos and AI Adoption',
    date: '2026-03-12',
    excerpt: 'I gave a live talk on AI agents to 1,400 registrants. The audience reaction confirmed what I see in every training room — people want proof, not promises.',
    tags: ['ai-adoption', 'ai-agents', 'speaking'],
    type: 'reflection',
    image: '/images/blog/openclaw-talk-cover.webp',
    featured: false,
  },
  {
    slug: 'renpho-ecommerce-ai-shenzhen',
    title: 'Three AI Gaps in an E-Commerce Team Already at 70-80% Adoption',
    date: '2026-03-11',
    excerpt: 'A Shenzhen visit exposed a pattern I see everywhere: teams using AI at 50% efficiency because nobody taught them model selection.',
    tags: ['e-commerce', 'ai-adoption'],
    type: 'case-study',
    image: '/images/blog/renpho-ecommerce-cover.webp',
    featured: false,
  },
  {
    slug: 'toy-company-takes-ai-seriously',
    title: 'What Happens When a Toy Company Takes AI Seriously',
    date: '2026-02-09',
    excerpt: 'Two sessions, 12 executives, and a gardening demo that secured buy-in. Inside an international toy company\'s AI training.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'case-study',
    image: '/images/blog/toy-company-ai-cover.webp',
    featured: false,
  },
  {
    slug: 'utility-company-seven-levels-ai-agents',
    title: 'Teaching a Utility Company the 7 Levels of AI Agents',
    date: '2025-12-23',
    excerpt: 'I taught a major Hong Kong utility\'s team a 7-level AI agent framework, from basic chatbots to CLI-based systems. Here\'s what a utility company needs to know about AI agents.',
    tags: ['ai-adoption', 'ai-agents', 'enterprise'],
    type: 'case-study',
    image: '/images/blog/utility-ai-agents-cover.webp',
    featured: false,
  },
  {
    slug: 'traffic-light-protocol-ai-safety',
    title: 'The Traffic Light Protocol: How I Teach AI Safety in 15 Minutes',
    date: '2026-03-12',
    excerpt: 'A 3-color data classification system that makes AI safety intuitive. Used across banking, retail, and 500-person webinars.',
    tags: ['ai-adoption', 'enterprise', 'ai-safety'],
    type: 'methodology',
    image: '/images/blog/traffic-light-protocol-cover.webp',
    featured: false,
  },
  {
    slug: 'zh-chillpo-how-i-talk-to-ai',
    title: '一個小時，由零到全自動 YouTube 頻道：我是怎樣與 AI 溝通的',
    date: '2026-03-11',
    excerpt: '2,100 行終端紀錄，一個小時：由一個壞掉的部署開始，與 AI 助理 Ada 建成全自動 lo-fi 音樂 YouTube 頻道。這篇不談技術，談溝通——何時沉默、何時用兩個字做決定、何時直接貼上錯誤訊息。',
    tags: ['ai-adoption', 'ai-tools'],
    type: 'reflection',
    language: 'zh-Hant',
    image: '/images/blog/chillpo-ai-conversation-cover.webp',
    featured: true,
  },
  {
    slug: 'ada-gersang-gamifying-claude-code',
    title: 'I Built an RTK-Style Strategy Game on Top of My AI Workflow',
    date: '2026-03-07',
    excerpt: 'Every tool call in my AI assistant logs to SQLite. So I turned it into a Romance of the Three Kingdoms-style strategy game -- with named skills, fractional XP, and a six-agent squad.',
    tags: ['ai-adoption', 'ai-tools'],
    type: 'reflection',
    image: '/images/blog/ada-gersang-cover.webp',
    featured: false,
  },
  {
    slug: 'i-built-voice-mode-claude-code',
    title: 'I Built a Voice Mode for Claude Code Because I Can\'t Read',
    date: '2026-03-07',
    excerpt: 'Long terminal output is attention-hostile for ADHD developers. So I built a plugin that speaks Claude Code responses aloud -- zero deps, one install command.',
    tags: ['ai-adoption', 'ai-tools'],
    type: 'reflection',
    image: '/images/blog/claude-voice-cover.webp',
    featured: false,
  },
  {
    slug: 'what-my-ai-drew-when-i-asked',
    title: 'How an Art Prompt Turned Into a 78-Bug Audit of My AI Assistant',
    date: '2026-03-06',
    excerpt: "I asked my AI to draw what's in its mind. The interesting part wasn't the drawing -- it was the working session that followed, where we found 78 logged misfires and shipped 14 fixes.",
    tags: ['ai-agents', 'claude-code', 'workflow', 'system-design'],
    type: 'reflection',
    image: '/images/blog/adas-mind-line-art.webp',
    featured: false,
  },
  {
    slug: 'how-to-design-ai-pioneer-program',
    title: 'How to Design a 6-Session AI Pioneer Program That Actually Changes Behavior',
    date: '2026-03-04',
    excerpt: 'How I designed a 6-session corporate AI training program for 19 HR professionals, using Copilot-only tools and a change management approach that saved 5-8 hours per week.',
    tags: ['enterprise', 'ai-adoption'],
    type: 'case-study',
    image: '/images/blog/pioneer-program-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-training-gap-managers-vs-workers',
    title: 'The AI Training Gap: 70% of Managers Want It, 14% of Workers Get It',
    date: '2026-02-24',
    excerpt: '70% of managers say an AI-trained workforce is critical, but only 14% of workers have been offered training. Here\'s what\'s going wrong.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'insight',
    image: '/images/blog/training-gap-cover.webp',
    featured: false,
  },
  {
    slug: 'it-department-ai-adoption-bottleneck',
    title: 'Why Your IT Department Is Your Biggest AI Adoption Bottleneck',
    date: '2026-02-17',
    excerpt: 'Enterprise AI adoption fails when IT blocks everything by default. How to turn IT from gatekeeper to enabler using a 3-tier security framework.',
    tags: ['ai-adoption', 'enterprise', 'ai-safety'],
    type: 'insight',
    image: '/images/blog/it-bottleneck-cover.webp',
    featured: false,
  },
  {
    slug: 'stop-telling-ai-be-creative',
    title: 'Stop Telling AI to \'Be Creative\': What 35 Prompting Strategies Actually Reveal',
    date: '2026-02-10',
    excerpt: 'Wharton researchers tested 35 prompting strategies and found that saying \'be creative\' is the least effective. Here\'s what actually works.',
    tags: ['prompts', 'ai-adoption'],
    type: 'insight',
    image: '/images/blog/stop-be-creative-cover.webp',
    featured: false,
  },
  {
    slug: '84-percent-never-used-ai',
    title: '84% of People Have Never Used AI. That\'s Your Competitive Advantage.',
    date: '2026-02-03',
    excerpt: '84% of people have never used AI and only 0.3% pay for it. The window of competitive advantage is wide open.',
    tags: ['ai-adoption', 'enterprise', 'leadership'],
    type: 'insight',
    image: '/images/blog/84-percent-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-agents-knowledge-work-field-notes',
    title: 'AI Agents Are Coming for Knowledge Work: A Trainer\'s Field Notes',
    date: '2026-01-27',
    excerpt: 'AI agents are moving from demos to deployments. What enterprise teams actually need to know about agentic workflows and what\'s still hype.',
    tags: ['ai-adoption', 'ai-agents', 'enterprise'],
    type: 'insight',
    image: '/images/blog/ai-agents-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-maturity-trap-stuck-stage-one',
    title: 'The AI Maturity Trap: Why Most Companies Are Stuck at Stage 1',
    date: '2026-01-20',
    excerpt: 'Most companies think they\'re behind on AI. They\'re right -- but the fix isn\'t more tools. The 4-stage maturity model and how to move past Stage 1.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'framework',
    image: '/images/blog/maturity-trap-cover.webp',
    featured: false,
  },
  {
    slug: 'ctf-ai-design-thinking-workshop-2026',
    title: 'Training the World\'s Largest Jewelry Retailer on AI x Design Thinking',
    date: '2026-01-17',
    excerpt: 'Insights from my third workshop with Chow Tai Fook, where cross-functional teams competed to create Go-To-Market campaigns using AI tools in a hackathon format.',
    tags: ['workshop'],
    type: 'case-study',
    image: '/images/blog/ctf-workshop-2026.jpeg',
    featured: false,
  },
  {
    slug: 'when-ceo-learns-to-code-ai',
    title: 'When the CEO Learns to Code: What AI Means for Leadership',
    date: '2026-01-13',
    excerpt: 'CEOs are building products with AI coding tools on weekends. What this means for enterprise AI training and the gap between leaders and teams.',
    tags: ['ai-adoption', 'leadership', 'enterprise'],
    type: 'insight',
    image: '/images/blog/ceo-codes-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-skill-formation-problem',
    title: 'The Skill Formation Problem: Is AI Making Us Worse at Our Jobs?',
    date: '2026-01-06',
    excerpt: 'Research shows AI can negatively impact skill formation. Why that finding matters and what responsible corporate AI training looks like.',
    tags: ['ai-adoption', 'ai-safety', 'enterprise'],
    type: 'insight',
    image: '/images/blog/skill-formation-cover.webp',
    featured: false,
  },
  {
    slug: 'why-i-teach-copilot-not-chatgpt',
    title: 'Why I Teach Copilot Instead of ChatGPT in Enterprise Workshops',
    date: '2025-12-30',
    excerpt: 'Most AI trainers default to ChatGPT. Here\'s why I start with Microsoft Copilot for enterprise workshops and how it changes adoption.',
    tags: ['ai-adoption', 'enterprise'],
    type: 'methodology',
    image: '/images/blog/copilot-not-chatgpt-cover.webp',
    featured: false,
  },
  {
    slug: 'ai-tool-stack-3000-professionals',
    title: 'The AI Tool Stack I Use to Train 3,000 Professionals a Year',
    date: '2025-12-22',
    excerpt: 'The complete AI tool stack an AI trainer uses across banking, retail, education, and engineering workshops -- from Copilot to NotebookLM.',
    tags: ['ai-adoption', 'ai-tools', 'enterprise'],
    type: 'methodology',
    image: '/images/blog/tool-stack-cover.webp',
    featured: false,
  },
  {
    slug: '1500-banking-professionals-ai-adoption',
    title: 'What 1,500 Banking Professionals Taught Me About AI Adoption',
    date: '2025-11-15',
    excerpt: 'Lessons from training employees at a major Hong Kong bank across 13 countries - on safety frameworks, workflow integration, and the 70/30 human-AI split.',
    tags: ['enterprise', 'ai-adoption', 'banking'],
    type: 'case-study',
    image: '/images/blog/banking-ai-adoption-cover.webp',
    featured: false,
  },
  {
    slug: '3-ai-tools-save-focus',
    title: '3 AI Tools That Saved My Focus (And Can Save Yours)',
    date: '2025-08-10',
    excerpt: 'Three specific AI tools that solve real attention problems: information triage, meeting recovery, and structured thinking. Based on my second appearance on Club 80.',
    tags: ['ai-adoption', 'ai-tools', 'productivity', 'speaking'],
    type: 'insight',
    image: '/images/blog/3-ai-tools-focus-cover.webp',
    featured: false,
    linkedPost: 'zh-3-ai-tools-save-focus',
  },
  {
    slug: 'zh-3-ai-tools-save-focus',
    title: '三個救回我專注力的 AI 工具：會八十第 049 集',
    date: '2025-08-10',
    excerpt: '專注力對有 ADHD 的 Sam 來說，是一天有產出還是報廢的分界。會八十第 049 集他分享三個工具：ChatGPT 分流電郵、AI 會議筆記、AI 思考夥伴——每個都移除一種特定的注意力流失。',
    tags: ['ai-adoption', 'ai-tools', 'productivity', 'speaking'],
    type: 'insight',
    language: 'zh-Hant',
    image: '/images/blog/3-ai-tools-focus-cover.webp',
    featured: false,
    linkedPost: '3-ai-tools-save-focus',
  },
  {
    slug: 'cts-tourism-ai-training',
    title: 'Why I Trained CTS Executives on Doubao Instead of ChatGPT',
    date: '2025-07-20',
    excerpt: 'For a tourism company operating across Hong Kong and mainland China, the standard Western AI toolkit was the wrong starting point. The tool decision turned out to be the strategic one.',
    tags: ['corporate-training', 'leadership', 'international', 'tourism'],
    type: 'case-study',
    image: '/images/blog/cts-ai-workshop.webp',
    featured: false,
  },
  {
    slug: 'why-pay-for-ai-tools',
    title: 'Why Free AI Isn\'t Enough: The Case for Paying for AI Tools',
    date: '2025-06-15',
    excerpt: '76% of viewers in a live poll said they don\'t pay for AI tools. Most of the 24% who do aren\'t using the features they paid for. The actual percentage using AI at full capability is vanishingly small.',
    tags: ['ai-adoption', 'ai-tools', 'speaking'],
    type: 'insight',
    image: '/images/blog/why-pay-ai-tools-cover.webp',
    featured: false,
    linkedPost: 'zh-why-pay-for-ai-tools',
  },
  {
    slug: 'zh-why-pay-for-ai-tools',
    title: '免費 AI 為何不夠用：我在會八十那晚講的實話',
    date: '2025-06-15',
    excerpt: '會八十第 024 集現場投票：498 人中 76% 不付費用 AI。大部分人未接觸過真正的官方產品，付費的人也鮮有碰過模型選擇器、Deep Search 和設定面板。真正把 AI 用盡的人，可能只有幾個百分點。',
    tags: ['ai-adoption', 'ai-tools', 'speaking'],
    type: 'insight',
    language: 'zh-Hant',
    image: '/images/blog/why-pay-ai-tools-cover.webp',
    featured: false,
    linkedPost: 'why-pay-for-ai-tools',
  },
  {
    slug: 'polyu-finance-ai-workflow',
    title: 'How PolyU\'s Finance Team Built an AI Workflow That Actually Works',
    date: '2025-05-15',
    excerpt: 'Building sustainable AI workflows for a university finance office - starting with strategy, establishing the ACE framework, and watching tool users become process designers.',
    tags: ['enterprise', 'ai-adoption', 'ai-education'],
    type: 'case-study',
    image: '/images/blog/polyu-ace-framework.jpeg',
    featured: false,
  },
  {
    slug: 'hkct-ai-education-workshop',
    title: "What 75 Minutes of AI Training Can and Can't Do for 400 Educators",
    date: '2025-04-27',
    excerpt: "HKCT put 400 staff in one room for 75 minutes of AI training. Some started experimenting within a week. Most didn't. Here's what one session honestly achieves.",
    tags: ['ai-education', 'corporate-training', 'ai-adoption', 'workshop'],
    type: 'case-study',
    image: '/images/blog/hkct-ai-workshop.jpg',
    featured: false,
  },
  {
    slug: 'hkjc-mt-ai-training',
    title: "Three Layers of a Better Prompt: What HKJC's Management Trainees Actually Practiced",
    date: '2025-04-26',
    excerpt: "The three-layer prompt framework I taught Hong Kong Jockey Club's management trainees -- Precision, Structure, Context -- with the actual before-and-after prompts from the workshop.",
    tags: ['corporate-training', 'prompts', 'ai-writing'],
    type: 'case-study',
    image: '/images/blog/hkjc-mt-workshop.jpeg',
    featured: false,
  },
  {
    slug: 'dorich-ai-efficiency-workshop',
    title: 'Four Prompt Patterns from a 2.5-Hour Community Workshop',
    date: '2025-04-02',
    excerpt: "When Dorich asked for an AI session that wouldn't be forgotten in a week, we built it around four specific prompts their audience could use the next morning.",
    tags: ['workshop', 'prompts', 'ai-adoption', 'productivity'],
    type: 'case-study',
    image: '/images/blog/dorich-ai-workshop.webp',
    featured: false,
  },
  {
    slug: 'arup-ai-lunch-learn',
    title: 'What Happens When 50 Engineers Get 45 Minutes with AI',
    date: '2025-03-28',
    excerpt: 'Introducing "C-How Thinking" at Arup - a framework that bridges left-brain logic and right-brain creativity through Copilot and Canva.',
    tags: ['enterprise', 'ai-adoption', 'system-design'],
    type: 'case-study',
    image: '/images/blog/arup-ai-workshop.jpg',
    featured: false,
  },
  {
    slug: 'welcome-to-my-blog',
    title: 'Welcome to My Blog',
    date: '2024-05-17',
    excerpt: 'Starting a space to share insights on AI adoption, workshop learnings, and the human side of technology.',
    tags: ['ai-adoption'],
    type: 'announcement',
    image: '/images/blog/welcome-blog-cover.webp',
    featured: false,
  },
];

// Guarantee reverse-chronological order at import time (newest first).
// ISO date strings (YYYY-MM-DD) sort lexicographically.
export default posts.slice().sort((a, b) => b.date.localeCompare(a.date));
